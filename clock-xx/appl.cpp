...
auto DoSteps(const std::string& steps) {
    std::ostringstream os;
    const auto secs = std::stoull(steps);
    clockWork.run(secs);
    os << "clockwork advancing " << secs << " seconds";
    return UI::Command::Result{ true, os.str() };
}
...
void TickShowActive() {
    for (auto& e : allTimers) {
        ...
        case State::Countdown:
            ...
            clockRef.TickDown();
            // ===================-
            std::cerr << "[C] " << clockRef << std::endl;
            if (clockRef.IsFloor()) {
                // - - - - - ->>>>>>>>>>>>>>-
                stateRef = State::Expired;
                // - - - - - ->>>>>>>>>>>>>>-
                std::cerr << "*** " << clockRef.GetName() << std::endl;
            }
            break;
        case State::Expired:
...
void appl() {
    clockWork.subscribe(KitchenTimer::TickShowActive);
    UI::main_loop(std::cin, std::cout, std::cout,
